...
{
    QByteArray header;
    unsigned char aa = 0xcd;
    header.append(aa); // cryptdevice byte
    ...
    QByteArray padding(kHeaderLength - header.length(), aa);
...
bool CryptFileDevice::tryParseHeader()
{
    ...
    QByteArray expectedPadding(padding.length(), aa);
...
bool CryptFileDevice::initCipher()
{
    ...
    unsigned char* iv = new unsigned char[ivLength];
